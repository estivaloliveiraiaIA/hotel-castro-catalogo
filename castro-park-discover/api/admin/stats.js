...
export default async function handler(req, res) {
  ...
  const [places, placesActive, placesRecommended, events, itineraries, partners] = await Promise.all([
    supabase.from("places").select("*", { count: "exact", head: true }),
    supabase.from("places").select("*", { count: "exact", head: true }).eq("is_active", true),
    supabase.from("places").select("*", { count: "exact", head: true }).eq("hotel_recommended", true),
  ...
  return res.status(200).json({
    ...
    places_active: placesActive.count ?? 0,
    places_recommended: placesRecommended.count ?? 0,
